...
safe_fromJSON <- function(path) {
  tryCatch({
    ...
    warning("Failed to parse file: ", path, " — ", conditionMessage(e))
...
system.time({
  ...
  B <- bind_rows(lapply(aug_files[1:3], json_to_df))
  })
unique(format(B$datetime, "%Y-%m-%d %H:%M"))

mar_json_data <- fromJSON(mar_files[1])
aug_json_data <- fromJSON(aug_files[1])
aug_json_data2 <- safe_fromJSON(aug_files[1])

aug_json_data$EVSEStatuses$OperatorID[[1]]
aug_json_data$EVSEStatuses[[7]]$EVSEStatusRecord
length(aug_json_data$EVSEStatuses)
```


```{r SANDBOX - process august 2025}
sub_folder <- "2025-08"
