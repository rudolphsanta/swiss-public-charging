...
chg_avail_files <- list.files(charging_data_root,pattern = "\\.parquet$",full.names = TRUE)
...
CH_EVSE_details <- read.csv("M:/Rudolph/R-Projects/swiss-public-charging/data/ChargingStationDetails_19-02-2026.csv",
                            colClasses = c(
...
sum(CH_EVSE_details$longitude > 180, na.rm = TRUE)
summary(CH_EVSE_details)
dim(CH_EVSE_details) #18299 rows

CH_EVSE_details$LastUpdate <- ymd_hms(CH_EVSE_details$LastUpdate, tz = "Europe/Zurich")
...
CH_EVSE_details <- CH_EVSE_details %>%
  filter(!is.na(longitude) & !is.na(latitude))

dim(CH_EVSE_details) #18155 rows. 144 rows eliminated
#remove any rows that are EXACT duplicates (duplicated across all columns)
duplicated_rows <- CH_EVSE_details[duplicated(CH_EVSE_details), ]

duplicated_rows_all <- CH_EVSE_details %>%
  group_by(across(everything())) %>%
  filter(n() > 1) %>%
  ungroup()

duplicated_summary <- CH_EVSE_details %>%
  group_by(across(everything())) %>%
  summarise(dup_count = n(), .groups = "drop") %>%
  filter(dup_count > 1)

summary(duplicated_summary$dup_count)

CH_EVSE_details <- CH_EVSE_details[!duplicated(CH_EVSE_details), ]

summary(CH_EVSE_details)
dim(CH_EVSE_details) #17893 rows. 262 more rows eliminated
...
# summary(CH_EVSE_details$ChargingFacility_power)
# sum(CH_EVSE_details$ChargingFacility_power == 0, na.rm = TRUE)
```
...
process_file <- function(file, IDs = NULL, status = NULL, year = NULL, month = NULL, day = NULL, wday = NULL, hour = NULL, minute = NULL, existing_dt = NULL) {
  ...
  if (is.na(finfo$size) || finfo$size == 0) {
    message("Skipping zero-byte parquet file: ", file)
    return(NULL)
  }
  #read the file (safe)
  dt <- tryCatch(
    ...
    error = function(e) {
      message("Skipping unreadable parquet file: ", file)
  ...
  if (nrow(dt) == 0 || !"datetime" %in% names(dt)) {
    message("Skipping empty or malformed parquet file: ", file)
    return(NULL)
  }
...
load_filter_charging_dt_parallel <- function(files_to_load, IDs = NULL, status = c("Available","Occupied"), year = NULL, month = NULL, day = NULL, wday = NULL, hour = NULL, minute = NULL, existing_dt = NULL, n_cores = 4, mem_efficient = FALSE, prefilter = TRUE) {
  rows_elim <- 0
  file_summaries <- list()
  # Ensure existing_dt is a data.table
  if (!is.null(existing_dt)) {
...
filter_IDs <- unique(CH_EVSE_details$EvseID)

#2022 summary
Rows_2022 <- load_filter_charging_dt_parallel(chg_avail_files,
                                              IDs = filter_IDs,
                                              status = NULL,
                                              year = 2022,
                                              n_cores = 4,
  ...
  count(EVSEStatus) %>%
  pivot_wider(
    names_from = EVSEStatus,
    values_from = n,
    values_fill = 0) %>%
  mutate(Total_Rows = rowSums(across(everything())),
         year = 2022) %>%
  select(year, Total_Rows, everything())

#2023 summary
Rows_2023 <- load_filter_charging_dt_parallel(chg_avail_files,
                                              IDs = filter_IDs,
                                              status = NULL,
                                              year = 2023,
                                              n_cores = 4,
...
#2024 summary
Rows_2024 <- load_filter_charging_dt_parallel(chg_avail_files,
                                              IDs = filter_IDs,
                                              status = NULL,
                                              year = 2024,
                                              n_cores = 4,
...
#2025 summary
Rows_2025 <- load_filter_charging_dt_parallel(chg_avail_files,
                                              IDs = filter_IDs,
                                              status = NULL,
                                              year = 2025,
                                              n_cores = 4,
...
All_year_summary <- rbind(Rows_2022,Rows_2023,Rows_2024,Rows_2025)
Total <- sum(All_year_summary$Total_Rows)
Removed <- sum(All_year_summary$Total_Rows) - (sum(All_year_summary$Available) + sum(All_year_summary$Occupied))
...
```{r generate a single typical day set of data for a selected power level of charger - DELETE}
system.time({
...
bootstrapped_hourly_summary <- function(dt, n_boot = 1000, seed = 123, sample.ret = FALSE) {
  ...
  result <- dt[, {
    ...
    boot_stats <- unlist(compute_stats(sample(prop_occupied, size = n_boot, replace = TRUE)))
...
```{r Sample distributions of one hour - DETELE Unnecessary}
...
utilization_visual <- function(dt_util,
  ...
  if (line) {
    ...
    p <- ggplot(dt_util, aes(x = factor(hour))) +
      geom_boxplot(
        aes(
          ymin = pmax(min_prop, q1_prop - (1.5*(q3_prop - q1_prop))),
          lower = q1_prop,
          middle = median_prop,
          upper = q3_prop,
          ymax = q3_prop + (1.5*(q3_prop - q1_prop))
        ),
        stat = "identity",
        fill = "skyblue",
        alpha = 0.4
...
write.csv(hourly_summary,"C:/Users/santar_r/Documents/Rudolph_local/Datasets/Swiss-EVChg-UtilizationRate_Summary/cross_section_summary.csv")
write.csv(hourly_summary_boot,"C:/Users/santar_r/Documents/Rudolph_local/Datasets/Swiss-EVChg-UtilizationRate_Summary/cross_section_summary_bootstrapped.csv")
write.csv(hourly_summary_agg,"C:/Users/santar_r/Documents/Rudolph_local/Datasets/Swiss-EVChg-UtilizationRate_Summary/cross_section_summary_means.csv")
write.csv(hourly_summary_agg_max,"C:/Users/santar_r/Documents/Rudolph_local/Datasets/Swiss-EVChg-UtilizationRate_Summary/cross_section_summary_maxes.csv")
write.csv(hourly_summary_hboot,"C:/Users/santar_r/Documents/Rudolph_local/Datasets/Swiss-EVChg-UtilizationRate_Summary/cross_section_summary_hbootstrapped.csv")
```


```{r visualization of cross-sectional data across the 5 methods, fig.width=18, fig.height=6}
...
#grouped boxplot
hourly_summary$method        <- "cross-sectional"
hourly_summary_agg$method    <- "means"
hourly_summary_agg_max$method<- "maximums"
hourly_summary_boot$method   <- "bootstrapped"
hourly_summary_hboot$method  <- "hierarchal-bootstrapped"

df_all <- bind_rows(
  hourly_summary,
  hourly_summary_agg,
  hourly_summary_agg_max,
  hourly_summary_boot,
  hourly_summary_hboot
)

df_all$hour   <- factor(df_all$hour, levels = 0:23)
df_all$method <- factor(df_all$method, 
                        levels = c("cross-sectional", "means", "maximums", "bootstrapped", "hierarchal-bootstrapped"))

df_all <- df_all %>%
  mutate(
    hour_num = as.numeric(as.character(hour)),  # numeric hours
    method = factor(method, levels = c("cross-sectional", "means", "maximums", "bootstrapped", "hierarchal-bootstrapped"))
  )

# Assign horizontal offsets for the 5 methods
method_offsets <- c("cross-sectional" = -0.4, "means" = -0.2, "maximums" = 0, "bootstrapped" = 0.2, "hierarchal-bootstrapped" = 0.4)
df_all <- df_all %>%
  mutate(dodge_x = hour_num + method_offsets[method])

#plot
ggplot(df_all, aes(x = hour, fill = method)) +
  geom_boxplot(aes(ymin   = pmax(min_prop, q1_prop - (1.5*(q3_prop - q1_prop))),
                   lower  = q1_prop,
                   middle = median_prop,
                   upper  = q3_prop,
                   ymax   = q3_prop + (1.5*(q3_prop - q1_prop))),
  ...
  labs(
    title = "Hourly Utilization Distributions Across Sampling Methods",
    x = "Hour of Day",
    y = "Utilization Rate (%)",
    fill = "Method"
  ) +
...
ggsave(filename = "M:/Rudolph/Manuscripts and Publications/Paper - Charging Dataset/Figures/five_sampling_methods_distribution.png", width = 14,height = 6,dpi = 300)


# Alternative: Plot IQRs as bars
# ggplot(df_all, aes(x = dodge_x, fill = method)) +
#   geom_rect(
#     aes(
#       xmin = dodge_x - 0.15,  # width of each bar
#       xmax = dodge_x + 0.15,
#       ymin = q1_prop,
#       ymax = q3_prop
#     ),
...
#   scale_fill_brewer(palette = "Set2") +
#   labs(
#     title = "Hourly Utilization IQRs Across Sampling Methods",
#     x = "Hour of Day",
#     y = "Utilization Rate",
#     fill = "Method"
#   ) +
...
#Visualize weekday and weekend hourly utilization rates
```{r weekdays}
hours <- 0:23
...
# --- Combine hourly results into final 5 tables ---
hourly_summary_WKday       <- rbindlist(stats_list, use.names = TRUE, fill = TRUE)

#visualize
utilization_visual(hourly_summary_WKday, title = "Hourly distibution of utilization rates for weekends")

#Manuscript FIGURE
# utilization_visual(hourly_summary_WKday, line = FALSE, legend = FALSE, title = "Hourly distibution of utilization rates for weekdays") 

ggplot(hourly_summary_WKday) +
...
ggsave(filename = "M:/Rudolph/Manuscripts and Publications/Paper - Charging Dataset/Figures/weekday_utilization_crosssection3.png", width = 8,height = 6,dpi = 300)
...
# --- Combine hourly results into final 5 tables ---
hourly_summary_WKend       <- rbindlist(stats_list, use.names = TRUE, fill = TRUE)

#visualize
# utilization_visual(hourly_summary_WKend, title = "Hourly distibution of all utilization rates")
#Manuscript FIGURE
# utilization_visual(hourly_summary_WKend, line = FALSE, legend = FALSE, title = "Hourly distibution of utilization rates for weekends") 

# ggsave(filename = "M:/Rudolph/Manuscripts and Publications/Paper - Charging Dataset/Figures/weekday_utilization_crosssection.png", width = 8,height = 6,dpi = 300)


ggplot(hourly_summary_WKend) +
  geom_boxplot(aes(x = factor(hour),
                   ymin   = pmax(min_prop, q1_prop - (1.5*(q3_prop - q1_prop))),
                   lower  = q1_prop,
                   middle = median_prop,
...
ggsave(filename = "M:/Rudolph/Manuscripts and Publications/Paper - Charging Dataset/Figures/weekend_utilization_crosssection3.png", width = 8,height = 6,dpi = 300)

cat("All hourly summaries completed.\n")
```

#Utilization Rate Validation against Gellrich et al. (2022)
```{r}
chg_dt_september <- load_filter_charging_dt_parallel(chg_avail_files,
                                                     IDs = NULL,
                                                     year = NULL,
                                                     month = 9,
                                                     day = NULL,
                                                     wday = NULL,
                                                     hour = NULL, # filter by this hour
                                                     minute = NULL,
                                                     existing_dt = NULL,
                                                     n_cores = 4,
                                                     mem_efficient = TRUE,
...
chg_dt_october <- load_filter_charging_dt_parallel(chg_avail_files,
                                                   IDs = NULL,
                                                   year = NULL,
                                                   month = 10,
                                                   day = NULL,
...
chg_dt_december <- load_filter_charging_dt_parallel(chg_avail_files,
                                                    IDs = NULL,
                                                    year = NULL,
                                                    month = 12,
                                                    day = NULL,
...
```{r plot data from same months for technical validation, fig.width=12, fig.height=6}
chg_dt_sample <- rbind(chg_dt_september, chg_dt_october, chg_dt_december)
chg_dt_sample <- chg_dt_sample[, .(utilization = mean(EVSEStatus == "Occupied")), by = datetime]
# -------------------------------------------------------
# YOUR INPUT DATA
# Assumed structure:
#   datetime : POSIXct timestamp
#   utilization : numeric utilization rate (%)
# -------------------------------------------------------

# Example:
# dt <- fread("your_real_data.csv")
...
chg_dt_sample[, `:=`(
  ...
  wday  = lubridate::wday(datetime, label = TRUE, week_start = 1)
)]

# Identify weekends (Saturday & Sunday)
chg_dt_sample[, weekday_type := ifelse(wday %in% c("Sat", "Sun"), "Weekend", "Weekday")]

# Filter for Sep, Oct, Dec
# dt3 <- dt[month %in% c("Sep", "Oct", "Dec")]


# -------------------------------------------------------
# Plot: 24-hour profiles for each day in Sep/Oct/Dec
# Colored by Weekday/Weekend
# Faceted by month with 72-hour total x-axis (3×24h)
# -------------------------------------------------------

# Create a 0–24 hour index that repeats for each month
chg_dt_sample[, hour_index := hour] 
plot_sample <- chg_dt_sample %>%
  filter(hour_index == 0) %>%
  mutate(hour_index = 24)
plot_sample <- rbind(chg_dt_sample,plot_sample) %>%
  filter(min == 0)

#Manuscript Figure
ggplot() +
  geom_line(data = plot_sample, 
            aes(x = hour_index, y = utilization,
                group = interaction(date, weekday_type),
  ...
  scale_color_manual(values = c("Weekday" = "#1B9E77", 
                                "Weekend" = "#D95F02")) +
  labs(title = "Hourly Utilization Rates – Weekdays vs Weekends",
...
ggsave(filename = "M:/Rudolph/Manuscripts and Publications/Paper - Charging Dataset/Figures/Sep_Oct_Dec-Utilization.png", width = 8,height = 8,dpi = 300)
...
#Subsets of data
```{r Utilization rate of 2023 Data}
output_folder <- file.path("C:/Users/santar_r/Documents/Rudolph_local/Datasets/Swiss-EVChg-UtilizationRate_Summary")
subset_name <- "2023"
...
# --- Combine hourly results into final 5 tables ---
hourly_summary_2023       <- rbindlist(stats_list, use.names = TRUE, fill = TRUE)
hourly_summary_boot_2023  <- rbindlist(boot_list, use.names = TRUE, fill = TRUE)
hourly_summary_agg_2023   <- rbindlist(agg_list, use.names = TRUE, fill = TRUE)
hourly_summary_agg_max_2023 <- rbindlist(agg_max_list, use.names = TRUE, fill = TRUE)
hourly_summary_hboot_2023 <- rbindlist(hboot_list, use.names = TRUE, fill = TRUE)

cat("All hourly summaries completed.\n")

write.csv(hourly_summary_2023,"C:/Users/santar_r/Documents/Rudolph_local/Datasets/Swiss-EVChg-UtilizationRate_Summary/2023_summary.csv")
write.csv(hourly_summary_boot_2023,"C:/Users/santar_r/Documents/Rudolph_local/Datasets/Swiss-EVChg-UtilizationRate_Summary/2023_summary_bootstrapped.csv")
write.csv(hourly_summary_agg_2023,"C:/Users/santar_r/Documents/Rudolph_local/Datasets/Swiss-EVChg-UtilizationRate_Summary/2023_summary_means.csv")
write.csv(hourly_summary_agg_max_2023,"C:/Users/santar_r/Documents/Rudolph_local/Datasets/Swiss-EVChg-UtilizationRate_Summary/2023_summary_maxes.csv")
write.csv(hourly_summary_hboot_2023,"C:/Users/santar_r/Documents/Rudolph_local/Datasets/Swiss-EVChg-UtilizationRate_Summary/2023_summary_hbootstrapped.csv")
```

```{r Utilization rate of 2024 Data}
output_folder <- file.path("C:/Users/santar_r/Documents/Rudolph_local/Datasets/Swiss-EVChg-UtilizationRate_Summary")
subset_name <- "2024"
...
# --- Combine hourly results into final 5 tables ---
hourly_summary_2024       <- rbindlist(stats_list, use.names = TRUE, fill = TRUE)
hourly_summary_boot_2024  <- rbindlist(boot_list, use.names = TRUE, fill = TRUE)
hourly_summary_agg_2024   <- rbindlist(agg_list, use.names = TRUE, fill = TRUE)
hourly_summary_agg_max_2024 <- rbindlist(agg_max_list, use.names = TRUE, fill = TRUE)
hourly_summary_hboot_2024 <- rbindlist(hboot_list, use.names = TRUE, fill = TRUE)

cat("All hourly summaries completed.\n")

write.csv(hourly_summary_2024,"C:/Users/santar_r/Documents/Rudolph_local/Datasets/Swiss-EVChg-UtilizationRate_Summary/2024_summary.csv")
write.csv(hourly_summary_boot_2024,"C:/Users/santar_r/Documents/Rudolph_local/Datasets/Swiss-EVChg-UtilizationRate_Summary/2024_summary_bootstrapped.csv")
write.csv(hourly_summary_agg_2024,"C:/Users/santar_r/Documents/Rudolph_local/Datasets/Swiss-EVChg-UtilizationRate_Summary/2024_summary_means.csv")
write.csv(hourly_summary_agg_max_2024,"C:/Users/santar_r/Documents/Rudolph_local/Datasets/Swiss-EVChg-UtilizationRate_Summary/2024_summary_maxes.csv")
write.csv(hourly_summary_hboot_2024,"C:/Users/santar_r/Documents/Rudolph_local/Datasets/Swiss-EVChg-UtilizationRate_Summary/2024_summary_hbootstrapped.csv")
```

```{r Utilization rate of 2025 Data}
output_folder <- file.path("C:/Users/santar_r/Documents/Rudolph_local/Datasets/Swiss-EVChg-UtilizationRate_Summary")
subset_name <- "2025"
...
# --- Combine hourly results into final 5 tables ---
hourly_summary_2025       <- rbindlist(stats_list, use.names = TRUE, fill = TRUE)
hourly_summary_boot_2025  <- rbindlist(boot_list, use.names = TRUE, fill = TRUE)
hourly_summary_agg_2025   <- rbindlist(agg_list, use.names = TRUE, fill = TRUE)
hourly_summary_agg_max_2025 <- rbindlist(agg_max_list, use.names = TRUE, fill = TRUE)
hourly_summary_hboot_2025 <- rbindlist(hboot_list, use.names = TRUE, fill = TRUE)

cat("All hourly summaries completed.\n")

write.csv(hourly_summary_2025,"C:/Users/santar_r/Documents/Rudolph_local/Datasets/Swiss-EVChg-UtilizationRate_Summary/2025_summary.csv")
write.csv(hourly_summary_boot_2025,"C:/Users/santar_r/Documents/Rudolph_local/Datasets/Swiss-EVChg-UtilizationRate_Summary/2025_summary_bootstrapped.csv")
write.csv(hourly_summary_agg_2025,"C:/Users/santar_r/Documents/Rudolph_local/Datasets/Swiss-EVChg-UtilizationRate_Summary/2025_summary_means.csv")
write.csv(hourly_summary_agg_max_2025,"C:/Users/santar_r/Documents/Rudolph_local/Datasets/Swiss-EVChg-UtilizationRate_Summary/2025_summary_maxes.csv")
write.csv(hourly_summary_hboot_2025,"C:/Users/santar_r/Documents/Rudolph_local/Datasets/Swiss-EVChg-UtilizationRate_Summary/2025_summary_hbootstrapped.csv")
```

```{r Utilization rate of Summer Weekday Data}
output_folder <- file.path("C:/Users/santar_r/Documents/Rudolph_local/Datasets/Swiss-EVChg-UtilizationRate_Summary")
subset_name <- "Summer_Weekday"
...
# --- Combine hourly results into final 5 tables ---
hourly_summary_SUMWK       <- rbindlist(stats_list, use.names = TRUE, fill = TRUE)
hourly_summary_boot_SUMWK  <- rbindlist(boot_list, use.names = TRUE, fill = TRUE)
hourly_summary_agg_SUMWK   <- rbindlist(agg_list, use.names = TRUE, fill = TRUE)
hourly_summary_agg_max_SUMWK <- rbindlist(agg_max_list, use.names = TRUE, fill = TRUE)
hourly_summary_hboot_SUMWK <- rbindlist(hboot_list, use.names = TRUE, fill = TRUE)

cat("All hourly summaries completed.\n")

write.csv(hourly_summary_SUMWK,"C:/Users/santar_r/Documents/Rudolph_local/Datasets/Swiss-EVChg-UtilizationRate_Summary/SUMWK_summary.csv")
write.csv(hourly_summary_boot_SUMWK,"C:/Users/santar_r/Documents/Rudolph_local/Datasets/Swiss-EVChg-UtilizationRate_Summary/SUMWK_summary_bootstrapped.csv")
write.csv(hourly_summary_agg_SUMWK,"C:/Users/santar_r/Documents/Rudolph_local/Datasets/Swiss-EVChg-UtilizationRate_Summary/SUMWK_summary_means.csv")
write.csv(hourly_summary_agg_max_SUMWK,"C:/Users/santar_r/Documents/Rudolph_local/Datasets/Swiss-EVChg-UtilizationRate_Summary/SUMWK_summary_maxes.csv")
write.csv(hourly_summary_hboot_SUMWK,"C:/Users/santar_r/Documents/Rudolph_local/Datasets/Swiss-EVChg-UtilizationRate_Summary/SUMWK_summary_hbootstrapped.csv")
```
...
##Charging Load
```{r}
chg_dt_load <- filter(chg_dt_full, EVSEStatus == "Occupied") %>%
  merge(CH_EVSE_details[, c("EvseID","ChargingFacility_power")], by = "EvseID")
...
load_minute_dt <- chg_dt_load[, .(total_load = sum(ChargingFacility_power, na.rm = TRUE),
                                  NA_sum     = sum(is.na(ChargingFacility_power)),
                                  mean_EVSE  = mean(ChargingFacility_power, na.rm = TRUE)), 
                              by = .(date, hour, minute)]

load_hour_dt <- load_minute_dt[, .(max_load = max(total_load, na.rm = TRUE),
                                   NA_max = max(NA_sum, na.rm = TRUE),
                                   mean_EVSE = mean(mean_EVSE)), 
                               by = .(date, hour)]
setorder(load_hour_dt,date,hour)


 
```

```{r generate the hourly load per month and save}
# if(exists("chg_dt_full")) rm(chg_dt_full) #clear the dt if it's there
output_root <- "C:/Users/santar_r/Documents/Rudolph_local/Datasets/Swiss-EVLoad-Data_CSV"
if(!dir.exists(output_root)) {
  dir.create(output_root, recursive = TRUE)
}

#set CH_EVSE_data such that it keeps only the higher power level if there are duplicate EvseIDs
chg_power_details <- CH_EVSE_details %>%
  group_by(EvseID) %>%
  slice_max(order_by = ChargingFacility_power, n = 1) %>%
  ungroup() %>%
  select(EvseID, ChargingFacility_power)

global_worst <- data.table() #initiate empty data table.

# month_parquet <- chg_avail_files[1]
for(month_parquet in chg_avail_files) {
  #generate the full output file path
  org_file_base <- basename(month_parquet) %>%
    tools::file_path_sans_ext(.)
  csv_filename <- paste0(org_file_base, "_load.csv")
  csv_filepath <- file.path(output_root, csv_filename)
  
  if(file.exists(csv_filepath)) {
    message("File already exists, skipping: ", csv_filepath)
    next
    }
  
  month_dt_full <- load_filter_charging_dt_parallel(month_parquet, mem_efficient = TRUE)
  
  if(is.null(month_dt_full)) {
    message("Empty file, skipping: ", csv_filepath)
    next
    }
  
  chg_dt_load <- filter(month_dt_full, EVSEStatus == "Occupied") %>%
    merge(chg_power_details, by = "EvseID")
  ...
  load_minute_dt <- chg_dt_load[, {
    total_load_val <- sum(ChargingFacility_power, na.rm = TRUE)
    count_occ_val  <- .N
    NA_sum_val     <- sum(is.na(ChargingFacility_power))
    mean_EVSE_val  <- mean(ChargingFacility_power, na.rm = TRUE)
    mean_imputed_val <- NA_sum_val * mean_EVSE_val
    .(total_load = total_load_val,
      # total_elc_kWh = total_load/20, #for 5-minute intervals, this is the integral
      count_occ  = count_occ_val,
      NA_sum     = NA_sum_val,
      pct_NA_points = NA_sum_val / (count_occ_val + NA_sum_val),
      mean_EVSE  = mean_EVSE_val,
      mean_imputed = mean_imputed_val,
      pct_imputed = mean_imputed_val / total_load_val)
    }, by = .(date, hour, minute)]
  
  load_hour_dt <- load_minute_dt[, `:=` (max_load = max(total_load, na.rm = TRUE),
                                         max_occ = max(count_occ, na.rm = TRUE),
                                         max_pct_NA_pts = max(pct_NA_points, na.rm = TRUE),
                                         NA_max = max(NA_sum, na.rm = TRUE),
                                         mean_EVSE = max(mean_EVSE),
                                         max_imputed = max(mean_imputed, na.rm = TRUE),
                                         max_pct_imputed = max(pct_imputed, na.rm = TRUE)), 
                                     by = .(date, hour)]
  setorder(load_hour_dt,date,hour)
  
  write.csv(load_hour_dt, csv_filepath)
  
  # (1) Worst NA count
  worst_NA_row <- load_minute_dt[which.max(NA_sum)][, source_file := month_parquet]

  # (2) Worst % NA points
  worst_pctNA_row <- load_minute_dt[which.max(pct_NA_points)][, source_file := month_parquet]

  # (3) Worst imputed power
  worst_imp_row <- load_minute_dt[which.max(mean_imputed)][, source_file := month_parquet]

  # (4) Worst % imputed
  worst_pctImp_row <- load_minute_dt[which.max(pct_imputed)][, source_file := month_parquet]

  # Combine into local worst
  local_worst_rows <- rbindlist(list(
    worst_NA_row,
    worst_pctNA_row,
    worst_imp_row,
    worst_pctImp_row
  ...
  global_worst <- rbind(global_worst, local_worst_rows, fill = TRUE)
}

#collapse the monthly global worsts into a true global worst.
final_global_worst <- rbindlist(list(
  global_worst[which.max(NA_max)],
  global_worst[which.max(max_pct_NA_pts)],
  global_worst[which.max(max_imputed)],
  global_worst[which.max(max_pct_imputed)]
), use.names = TRUE, fill = TRUE)

# Remove duplicates if different metrics point to the same row
final_global_worst <- unique(final_global_worst)
...
```{r load charging datasets}
# if(exists("all_month_load")) rm(all_month_load) #Start from scratch
chg_folder <- file.path("C:/Users/santar_r/Documents/Rudolph_local/Datasets/Swiss-EVLoad-Data_CSV")

for(month in list.files(chg_folder)) {
  # Extract year-month from filename e.g. "2025-10_charging_load.csv" -> "2025-10"
  file_month <- sub("^(\\d{4}-\\d{2})_.*\\.csv$", "\\1", month)
  
  # Skip if this month is already loaded
  if(exists("all_month_load") && file_month %in% format(as.Date(all_month_load$date), "%Y-%m")) {
    message("Skipping ", month, ": already present.")
    next
  }
  
  this_path <- file.path(chg_folder, month) 
  this_month <- read.csv(this_path) %>%
    select(-1) #%>% #remove the index column
    # mutate(date = as.Date(date, tz = "Europe/Zurich"))
  if(!exists("all_month_load")) {
    all_month_load <- this_month
  } else {
    all_month_load <- rbind(all_month_load, this_month)
  }
}

summary(as.Date(all_month_load$date))

```

```{r visualize load profile}
chg_load_dt <- setDT(all_month_load) 
chg_load_dt <- chg_load_dt[minute == 0]
chg_load_dt[ , `:=` (month = as.factor(month(date)),
                     Year = as.factor(year(date)), 
                     weekday = lubridate::wday(date, label = TRUE, abbr = FALSE, week_start = 1))]
chg_load_dt <- chg_load_dt %>%  
  mutate(day_type = as.factor(case_match(weekday,
                                         c("Monday","Tuesday","Wednesday","Thursday","Friday") ~ "Weekday",
                                         c("Saturday","Sunday") ~ "Weekend")))

#Manuscript Figure
ggplot(chg_load_dt) +
  geom_line(aes(x = hour, y = total_load/1000, color = Year, group = interaction(month,Year,date)), alpha = 0.5) +
  # xlim(0,23) +
  # ylim(0,100) +
  scale_x_continuous(breaks = c(0,4,8,12,16,20), limits = c(0,23), expand = c(0,0)) +
...
# ggsave(filename = "M:/Rudolph/Manuscripts and Publications/Paper - Charging Dataset/Figures/charging_load.png", width = 8,height = 6,dpi = 300)

```

```{r charging load validation against overall Swiss load}
Swiss_ELC_2022 <- readxl::read_excel("M:/Rudolph/R-Projects/swiss-public-charging/data/EnergieUebersichtCH-2022.xlsx", sheet = "Zeitreihen0h15")[-1,] %>%
  select(1,2) %>%
  rename("timestamp" = 1, "total_energy_cons_kWh" = 2) %>%
  mutate(timestamp = as.POSIXct(timestamp, tz = "Europe/Zurich", format = "%d.%m.%Y %H:%M")) %>%
  group_by(timestamp = floor_date(timestamp, "hour")) %>%
  summarise(hr_cons_MWh = sum(as.numeric(total_energy_cons_kWh)/1000, na.rm = TRUE),.groups = "drop")

Swiss_ELC_2023 <- readxl::read_excel("M:/Rudolph/R-Projects/swiss-public-charging/data/EnergieUebersichtCH-2023.xlsx", sheet = "Zeitreihen0h15")[-1,] %>%
  select(1,2) %>%
  rename("timestamp" = 1, "total_energy_cons_kWh" = 2) %>%
  mutate(timestamp = as.POSIXct(timestamp, tz = "Europe/Zurich", format = "%d.%m.%Y %H:%M")) %>%
  group_by(timestamp = floor_date(timestamp, "hour")) %>%
...
Swiss_ELC_2024 <- readxl::read_excel("M:/Rudolph/R-Projects/swiss-public-charging/data/EnergieUebersichtCH-2024.xlsx", sheet = "Zeitreihen0h15")[-1,] %>%
  select(1,2) %>%
  rename("timestamp" = 1, "total_energy_cons_kWh" = 2) %>%
  mutate(timestamp = as.POSIXct(timestamp, tz = "Europe/Zurich", format = "%d.%m.%Y %H:%M")) %>%
  group_by(timestamp = floor_date(timestamp, "hour")) %>%
...
Swiss_ELC_2025 <- readxl::read_excel("M:/Rudolph/R-Projects/swiss-public-charging/data/EnergieUebersichtCH-2025.xlsx", sheet = "Zeitreihen0h15")[-1,] %>%
  select(1,2) %>%
  rename("timestamp" = 1, "total_energy_cons_kWh" = 2) %>%
  mutate(timestamp = as.POSIXct(timestamp, tz = "Europe/Zurich", format = "%d.%m.%Y %H:%M")) %>%
  group_by(timestamp = floor_date(timestamp, "hour")) %>%
...
Swiss_ELC_ALL <- rbind(Swiss_ELC_2022, Swiss_ELC_2023, Swiss_ELC_2024, Swiss_ELC_2025)
```

```{r}
Swiss_ELC_dt <- rbind(Swiss_ELC_2022, Swiss_ELC_2023, Swiss_ELC_2024, Swiss_ELC_2025) %>%
  setDT() %>%
  mutate(
    date = as.character(as.Date(timestamp, tz = "Europe/Zurich")),
    hour = as.integer(format(timestamp, "%H"))) %>%
  group_by(date, hour) %>%
  summarise(hr_cons_MWh = max(hr_cons_MWh), .groups = "drop") #duplicate hours for daylight savings



# Swiss_ELC_dt[ , `:=` (date = ymd(timestamp),
#                       hour = as.integer(hour(timestamp)), 
#                       minute = as.integer(minute(timestamp)),
#                       weekday = lubridate::wday(timestamp, label = TRUE, abbr = FALSE, week_start = 1))]
# 
# Swiss_ELC_dt <- Swiss_ELC_dt %>%  
#   mutate(day_type = as.factor(case_match(weekday,
                                         # c("Monday","Tuesday","Wednesday","Thursday","Friday") ~ "Weekday",
                                         # c("Saturday","Sunday") ~ "Weekend")))
...
```{r join charging load with Swiss ELC system load}
# uniqueN(Swiss_ELC_dt$date)
# uniqueN(all_month_load$date)

load_hourly <- all_month_load %>%
  group_by(date, hour) %>%
  slice(1) %>%
  ungroup() %>%
  rename(max_EVload_kWh = max_load)

# Step 2: Right join so Swiss load hours are always retained
hr_load_all <- load_hourly %>%
  right_join(
    Swiss_ELC_dt %>% select(date, hour, hr_cons_MWh),
    by = c("date", "hour")
  ) %>%
  mutate(
    max_chg_pct_kWh = (max_EVload_kWh / 1000) / hr_cons_MWh
  ) %>%
  select(date, hour, max_EVload_kWh, hr_cons_MWh, max_chg_pct_kWh,
         mean_EVSE, max_occ, max_imputed, max_pct_imputed, NA_max)
...
df <- df %>%
  ...
  mutate(month   = lubridate::month(date, label = TRUE, abbr = TRUE),
         ...
         season  = case_when(
           lubridate::month(date) %in% c(11, 12, 1) ~ "Winter",
           lubridate::month(date) %in% c(2, 3, 4)  ~ "Spring",
           lubridate::month(date) %in% c(5, 6, 7)  ~ "Summer",
           lubridate::month(date) %in% c(8, 9, 10) ~ "Autumn"),
         season  = factor(season, levels = c("Winter", "Spring", "Summer", "Autumn")),
         day_type = if_else(lubridate::wday(date, week_start = 1) <= 5, "Weekday", "Weekend"))


# ── Figure 1: Dual-axis time series — EV load & system load ──────────────────
# Aggregate to daily max for readability over 3 years
daily <- df %>%
  mutate(date = as.Date(date)) %>%
  group_by(date) %>%
  summarise(
    # Use NA-aware logic: if ALL hours in the day have NA EV load, result is NA
    # otherwise take the daily maximum of non-NA values
    ev_load_mwh  = if_else(
      all(is.na(max_EVload_kWh)),
      NA_real_,
      max(max_EVload_kWh, na.rm = TRUE) / 1000),
    sys_load_mwh = if_else(
      all(is.na(hr_cons_MWh)),
      NA_real_,
      max(hr_cons_MWh, na.rm = TRUE)),
    pct_ev = if_else(
      all(is.na(max_chg_pct_kWh)),
      NA_real_,
      max(max_chg_pct_kWh, na.rm = TRUE)),
    .groups = "drop")
...
```{r manuscript figure}
primary_breaks   <- seq(0, 10000, by = 2000)
secondary_breaks <- seq(0, 100,   by = 20)

# Scale factor derived directly from fixed limits
scale_factor <- 10000 / 100   # = 100

#Plot
ggplot(data = daily, aes(x = date)) +
  geom_line(aes(y = sys_load_mwh, colour = "Total System Load"),
            linewidth = 0.4, alpha = 0.7) +
  geom_line(aes(y = ev_load_mwh * scale_factor, colour = "EV Charging Load"),
            linewidth = 0.5, alpha = 0.9) +
  scale_y_continuous(
    name    = "Total Swiss System Load (MW)",
    labels  = comma,
    breaks  = primary_breaks,
    limits  = c(0, max(primary_breaks)),
    expand   = expansion(mult = c(0, 0.02)),  # no padding at bottom, small at top
    sec.axis = sec_axis(
      ~ . / scale_factor,
      name   = "EV Charging Load (MW)",
      breaks = secondary_breaks,
      labels = comma(secondary_breaks))) +
  scale_colour_manual(
    values = c("Total System Load" = "grey50", "EV Charging Load" = "#D55E00")) +
  scale_x_date(
    date_breaks = "6 months",
    date_labels = "%b %Y",
    limits      = as.Date(c("2022-12-01", "2025-12-31")),
    expand      = c(0, 0)
  ) +
  labs(
    x      = NULL,
    colour = NULL,
    title  = "Daily Peak EV Charging Load vs. Total Swiss System Load") +
...
```{r manuscript figure}
# ── Figure: EV load as % of system load — full hourly time series ──────────
# Daily mean % with loess trend
# Apply same NA-robust daily aggregation
daily_pct <- df %>%
  mutate(date = as.Date(date)) %>%
  group_by(date) %>%
  summarise(
    mean_pct = {
      vals <- max_chg_pct_kWh[!is.na(max_chg_pct_kWh)] * 100
...
max_val  <- max(daily_pct$max_pct, na.rm = TRUE)
max_date <- daily_pct$date[which.max(daily_pct$max_pct)]

ggplot(daily_pct, aes(x = date)) +
  geom_line(aes(y = max_pct), colour = "grey75", linewidth = 0.35,
            alpha = 0.6) +
  # Smooth uses only non-NA rows so it fits continuously over the gap
  geom_smooth(data = daily_pct %>% filter(!is.na(max_pct)),
              aes(y = max_pct), method = "loess", span = 0.15,
              se = TRUE, colour = "#0072B2", fill = "#0072B2",
              alpha = 0.15, linewidth = 0.9) +
  annotate("text", x = max_date, y = max_val,
           label = sprintf("Max: %.2f%%  ", max_val),
           hjust = 1, vjust = 0, size = 4, colour = "#D55E00") +
  annotate("point", x = max_date, y = max_val,
  ...
  labs(
    x     = NULL,
    y     = "EV Charging Load as % of Swiss System Load",
    title = "EV Charging Load Share of Total Swiss Electricity System Load") +
  theme_bw(base_size = 11) +
  theme(panel.grid.minor = element_blank())
...
```{r manuscript figure}
# ── Figure 3: Diurnal % share by season and day type ─────────────────────────
diurnal <- df %>%
  filter(
    date >= as.Date("2022-12-01"),   # exclude pre-dataset dates
    !is.na(max_chg_pct_kWh)) %>%         # exclude gap months from averages
  group_by(season, day_type, hour) %>%
  summarise(
    mean_pct   = mean(max_chg_pct_kWh * 100, na.rm = TRUE),
    median_pct = median(max_chg_pct_kWh * 100, na.rm = TRUE),
    p25        = quantile(max_chg_pct_kWh * 100, 0.25, na.rm = TRUE),
    p75        = quantile(max_chg_pct_kWh * 100, 0.75, na.rm = TRUE),
    .groups    = "drop"  )

ggplot(diurnal, aes(x = hour, colour = day_type, fill = day_type)) +
  geom_ribbon(aes(ymin = p25, ymax = p75), alpha = 0.15, colour = NA) +
  geom_line(aes(y = mean_pct), linewidth = 0.8) +
  ...
  scale_y_continuous(
    ...
    expand = expansion(mult = c(0, 0.02))) +
  scale_colour_manual(values = c("Weekday" = "#0072B2", "Weekend" = "#E69F00")) +
  scale_fill_manual(values  = c("Weekday" = "#0072B2", "Weekend" = "#E69F00")) +
  labs(
    x      = "Hour of Day",
    y      = "EV Load as % of Swiss System Load",
    colour = "Day Type",
    fill   = "Day Type",
    title  = "Diurnal Profile of EV Charging Load Share by Season") +
  theme_bw(base_size = 11) +
  theme(
    legend.position  = "bottom",
    panel.grid.minor = element_blank(),
...
```{r}
# ── Table: Summary statistics of % share ─────────────────────────────────────
summary_tbl <- df %>%
  summarise(
    `Mean (%)`       = round(mean(max_chg_pct_kWh*100, na.rm = TRUE), 3),
    `Median (%)`     = round(median(max_chg_pct_kWh*100, na.rm = TRUE), 3),
    `Std. Dev. (%)`  = round(sd(max_chg_pct_kWh*100, na.rm = TRUE), 3),
    `95th pct. (%)` = round(quantile(max_chg_pct_kWh*100, 0.95, na.rm = TRUE), 3),
    `99th pct. (%)` = round(quantile(max_chg_pct_kWh*100, 0.99, na.rm = TRUE), 3),
    `Max (%)`        = round(max(max_chg_pct_kWh*100, na.rm = TRUE), 3),
    `N hours > 1%`   = sum(max_chg_pct_kWh*100 > 1, na.rm = TRUE),
    `N hours > 2%`   = sum(max_chg_pct_kWh*100 > 2, na.rm = TRUE),
    `Total hours`    = sum(!is.na(max_chg_pct_kWh*100))
  ) %>%
  pivot_longer(everything(), names_to = "Statistic", values_to = "Value")

print(summary_tbl)
# Export if needed:
# write.csv(summary_tbl, "ev_load_share_summary.csv", row.names = FALSE)
```
...
```{r}
ggplot(chg_load_dt) +
  geom_line(aes(x = hour, y = (max_load + max_imputed)/1000, color = Year, group = interaction(month,Year,date)), alpha = 0.5) +
  # xlim(0,23) +
  # ylim(0,100) +
  scale_x_continuous(breaks = c(0,4,8,12,16,20), limits = c(0,23), expand = c(0,0)) +
  scale_y_continuous(limits = c(0, 100), expand = c(0,0)) +
...
# ── Step 1: Monthly EVSE count and utilization rate ──────────────────────────
# Iterate over years and months, loading each month's data to extract:
#   - n_evse_total: distinct EVSEs observed (Available + Occupied)
#   - n_evse_occupied: distinct EVSEs observed as Occupied
#   - mean_util: mean prop_occupied across all snapshots in the month

expected_months <- data.frame(
  yr = rep(2022:2025, each = 12),
  mo = rep(1:12, times = 4)) %>%
  filter(!(yr == 2022 & mo < 12)) %>%  # start at Dec 2022
  mutate(key = as.character(paste(yr, mo, sep = "-")))

#find months not already in the list
already_processed <- as.character(names(monthly_coverage_list))
months_to_process <- expected_months %>%
  filter(!key %in% already_processed)

# monthly_coverage_list <- list()

for (yr in years) {
  for (mo in months) {
    if (yr == 2022 & mo < 12) next
    if (yr > 2025) next #only December 2022 to December 2025
    
    cat("Processing", yr, "-", sprintf("%02d", mo), "\n")
    
    dt_month <- load_filter_charging_dt_parallel(
      chg_avail_files,
      IDs     = NULL,
      status  = NULL,          # load ALL statuses to count total EVSEs
      year    = yr,
      month   = mo,
      n_cores = 4,
      mem_efficient = TRUE,
    ...
    if (is.null(dt_month) || nrow(dt_month) == 0) next
    
    # Count distinct EVSEs by status and compute utilization per snapshot
    # prop_occupied per snapshot (same approach as your summary_dt construction)
    snap_util <- dt_month[EVSEStatus %in% c("Available", "Occupied"),
                          .(prop_occupied = mean(EVSEStatus == "Occupied")),
                          by = datetime]
    
    monthly_coverage_list[[paste(yr, mo, sep = "-")]] <- data.table(
      year           = yr,
      month          = mo,
      month_date     = as.Date(paste(yr, mo, "01", sep = "-")),
      n_snapshots     = uniqueN(snap_util$datetime),
      n_evse_all      = uniqueN(dt_month$EvseID),
      n_evse_total   = dt_month[EVSEStatus %in% c("Available", "Occupied"),
                                uniqueN(EvseID)],
      n_evse_occupied = dt_month[EVSEStatus == "Occupied",
                                 uniqueN(EvseID)],
      mean_util      = mean(snap_util$prop_occupied, na.rm = TRUE),
      median_util    = median(snap_util$prop_occupied, na.rm = TRUE),
      p25_util       = quantile(snap_util$prop_occupied, 0.25, na.rm = TRUE),
      p75_util       = quantile(snap_util$prop_occupied, 0.75, na.rm = TRUE)
    )
    
    rm(dt_month, snap_util)
    gc()
  }
}

monthly_coverage <- rbindlist(monthly_coverage_list)

write.csv(monthly_coverage, "M:/Rudolph/0-Datasets/Switzerland/monthly_coverage.csv")
```


```{r}
if(!exists("monthly_coverage")) monthly_coverage <- read.csv("M:/Rudolph/0-Datasets/Switzerland/monthly_coverage.csv")

# ── Step 2: Monthly load and load share from hr_load_all ─────────────────────
# hr_load_all already has: date, hour, max_EVload_kWh, hr_cons_MWh, max_chg_pct_kWh
# Aggregate to monthly means

monthly_load <- as.data.table(hr_load_all) %>%
  mutate(date = as.Date(date)) %>%
  mutate(month_date = floor_date(date, "month")) %>%
  group_by(month_date) %>%
  summarise(
    mean_load_mw  = mean(max_EVload_kWh / 1000, na.rm = TRUE),
    mean_pct      = mean(max_chg_pct_kWh * 100, na.rm = TRUE),
    .groups = "drop"
  ) %>%
...
# ── Step 3: Join into one monthly summary dataframe ──────────────────────────
monthly_coverage$month_date <- as.Date(monthly_coverage$month_date)

monthly_fig <- monthly_coverage %>%
  left_join(monthly_load, by = "month_date") 
  filter(!is.na(mean_load_mw))   # drop months with no load data

monthly_fig <- monthly_fig %>%
  mutate(
    days_in_month     = as.numeric(days_in_month(month_date)),
    expected_snaps    = days_in_month * 24 * 12,          # 5-min intervals
    pct_snaps         = (n_snapshots / expected_snaps) * 100,
    pct_snaps_missing = 100 - pct_snaps)
sum(monthly_fig$expected_snaps) #324,576
sum(monthly_fig$n_snapshots) #301,342. 93%

```


```{r manuscript figure}
# ── Flag months with >50% missing snapshots ───────────────────────────────────
monthly_fig2 <- monthly_fig %>%
  mutate(
    poor_coverage = pct_snaps < 50,   # TRUE if more than 50% snapshots missing
    # For lines: set EVSE values to NA for poor coverage months
    n_evse_all_plot      = if_else(poor_coverage, NA_real_, as.numeric(n_evse_all)),
    n_evse_total_plot    = if_else(poor_coverage, NA_real_, as.numeric(n_evse_total)),
    n_evse_occupied_plot = if_else(poor_coverage, NA_real_, as.numeric(n_evse_occupied)),
    # Missing bar height: 100% missing = 5000, scaled linearly
    missing_bar_height   = (pct_snaps_missing / 100) * 5000,
    # For gap months (n_snapshots == 0): set to NA to show as gap
    # For poor coverage months (>50% missing but some data): retain values
    mean_util_plot   = if_else(n_snapshots == 0L, NA_real_, mean_util),
    median_util_plot = if_else(n_snapshots == 0L, NA_real_, median_util),
    p25_util_plot    = if_else(n_snapshots == 0L, NA_real_, p25_util),
    p75_util_plot    = if_else(n_snapshots == 0L, NA_real_, p75_util),
    # Yellow highlight for poor coverage months — ribbon at full height
    poor_cov_ymin    = if_else(poor_coverage & n_snapshots > 0L, 0,      NA_real_),
    poor_cov_ymax    = if_else(poor_coverage & n_snapshots > 0L, Inf,    NA_real_))
```


```{r manuscript figure}
# ── Panel 0: Missing snapshot % only (replaces secondary axis from pA) ────────
p0 <- ggplot(monthly_fig2, aes(x = month_date)) +
  geom_col(aes(y = pct_snaps_missing),
           fill = "#CC3333", alpha = 0.7, width = 25) +
  ...
  scale_y_continuous(
    ...
    expand = expansion(mult = c(0, 0)),
    labels = function(x) paste0(x, "%")) +
  labs(x = NULL, title = "Dataset Coverage, EV Charging Load Share, and Utilization Rate") +
  theme_bw(base_size = 11) +
  theme(
    panel.grid.minor = element_blank())


# ── Panel A: Total active EVSEs and max occupied only ─────────────────────────
pA <- ggplot(monthly_fig2, aes(x = month_date)) +
  # Yellow background for poor coverage months
  geom_rect(
    data = monthly_fig2 %>% filter(poor_coverage & n_snapshots > 0L),
    aes(xmin = month_date - 15, xmax = month_date + 15,
        ymin = -Inf, ymax = Inf),
    fill = "#E69F00", alpha = 0.2, inherit.aes = FALSE) +
  # Total active EVSEs as area + line in grey
  geom_area(aes(y = n_evse_total_plot),
            fill = "grey70", alpha = 0.5) +
  geom_line(aes(y = n_evse_total_plot, colour = "Total active EVSEs"),
            linewidth = 0.8) +
  geom_point(aes(y = n_evse_total_plot, colour = "Total active EVSEs"),
             size = 1.8) +
  # Max occupied EVSEs
  geom_line(aes(y = n_evse_occupied_plot, colour = "Max occupied EVSEs"),
            linewidth = 0.6, linetype = "dashed") +
  geom_point(aes(y = n_evse_occupied_plot, colour = "Max occupied EVSEs"),
             size = 1.8) +
  scale_colour_manual(
    values = c("Max occupied EVSEs" = "#D55E00",
               "Total active EVSEs" = "grey40")) +
  scale_x_date(
    date_breaks = "6 months",
    date_labels = "%b %Y",
    limits      = as.Date(c("2022-12-01", "2025-12-31")),
  ...
  scale_y_continuous(
    ...
    limits = c(0, 20000),
    expand = expansion(mult = c(0, 0.02))) +
  labs(x = NULL, colour = NULL, title = NULL) +
  theme_bw(base_size = 11) +
  theme(
...
# ── Panel B: Utilization rate with yellow highlights ─────────────────────────
pB <- ggplot(monthly_fig2, aes(x = month_date)) +
  # Yellow background for poor coverage months
  geom_rect(
    data = monthly_fig2 %>% filter(poor_coverage & n_snapshots > 0L),
    aes(xmin = month_date - 15, xmax = month_date + 15,
  ...
  geom_ribbon(aes(ymin = p25_util_plot, ymax = p75_util_plot),
              fill = "#009E73", alpha = 0.2) +
  geom_line(aes(y = mean_util_plot, colour = "Mean"),
            linewidth = 0.9) +
  geom_line(aes(y = median_util_plot, colour = "Median"),
            linewidth = 0.6, linetype = "dashed") +
  scale_colour_manual(values = c("Mean"   = "#009E73",
                                 "Median" = "#005a3c")) +
  scale_x_date(
    date_breaks = "6 months",
    date_labels = "%b %Y",
