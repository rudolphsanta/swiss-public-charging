...
ggsave(filename = "M:/Rudolph/Manuscripts and Publications/Paper - Charging Dataset/Figures/weekend_utilization_crosssection.png", width = 8,height = 6,dpi = 300)
...
```{r}
#what percent of load does the hour with the most NA EVSE potentially account for if we impute those NAs with the mean power level?
imputed_missing_load <- global_na_worst$NA_max*global_na_worst$mean_EVSE
pct_NA_load <- imputed_missing_load/global_na_worst$max_load
```
...
chg_load_dt[ , date := as.factor(date)]

ggplot(chg_load_dt, aes(x = hour, y = mean_EVSE, color = date, group = date)) +
  geom_line() +
  geom_point() +
  scale_x_continuous(breaks = 0:23) +
  labs(
    title = "Hourly Load (mean_EVSE)",
    x = "Hour of Day",
    y = "mean_EVSE"
  ) +
  theme_minimal() + 
