...
```{r SANDBOX loading Parquet files}
# undebug(load_charging_df)
start <- Sys.time()
# 
# A <- load_charging_df(chg_avail_files[1:2])
# df_time <- Sys.time()
B <- load_charging_dt(chg_avail_files[1:5])
dt_time <- Sys.time()

# difftime(df_time, start, units = "mins")
difftime(dt_time, start, units = "mins")

# print(format(object.size(A),units = "GB"))
print(format(object.size(B),units = "GB"))

# summary(B)
head(B)

# A_clean <- A[A$EVSEStatus %in% c("Available","Occupied"), ]
B_clean <- B[B$EVSEStatus %in% c("Available","Occupied"), ]
print(format(object.size(B_clean),units = "GB"))
# B_clean2 <- select(B_clean, datetime, EvseID, EVSEStatus)
# print(format(object.size(B_clean),units = "GB"))
# print(format(object.size(B_clean2),units = "GB"))
...
process_file <- function(file, IDs = NULL, status = c("Available","Occupied"), year = NULL, month = NULL, day = NULL, wday = NULL, hour = NULL, minute = NULL, existing_dt = NULL) {
...
load_filter_charging_dt_parallel <- function(files_to_load, IDs = NULL, status = c("Available","Occupied"), year = NULL, month = NULL, day = NULL, wday = NULL, hour = NULL, minute = NULL, existing_dt = NULL, n_cores = 4) {
  rows_elim <- 0
  file_summaries <- list()
  # Ensure existing_dt is a data.table
  if (!is.null(existing_dt)) {
  ...
  clusterExport(cl, varlist = c("files_to_load","IDs","status","year","month",
                                "day","wday","hour","minute","existing_dt"),
                envir = environment())
...
```{r create a single charging object in R - trial}
system.time({
  ...
  # rm(chg_dt_full)
  filter_IDs <- unique(mid_power_EVSE$EvseID)
...
system.time({
  ...
  # filter_IDs <- unique(mid_power_EVSE$EvseID)
  # undebug(load_filter_charging_dt_parallel)
  chg_dt_Rows <- load_filter_charging_dt_parallel(chg_avail_files[1],
                                                  status = NULL,
                                                  year = NULL,
                                                  n_cores = 8)
})
...
```{r SANDBOX}
system.time({
  utilization_rate(chg_dt_full, hour = 1)
})

head(unique(chg_dt_full$datetime))
...
compute_stats <- function(x) {
  ...
  q1_prop     <- quantile(x, 0.25, na.rm = TRUE)
  median_prop <- median(x, na.rm = TRUE)
  mean_prop   <- mean(x, na.rm = TRUE)
  q3_prop     <- quantile(x, 0.75, na.rm = TRUE)
...
##Using bootstrapping
bootstrapped_hourly_summary <- function(dt, n_boot = 1000, seed = 123) {
  set.seed(seed)
...
utilization_visual <- function(dt_util,
  ...
  if (line) {
    ...
    # Expect columns for q1_prop, median_prop, q3_prop, min_prop, max_prop
...
```{r function to convert charging availability tables - monthly data - DELETE}
...
```{r function to cycle through files in a folder - monthly data - DELETE}
