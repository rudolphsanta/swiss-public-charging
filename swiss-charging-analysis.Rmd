...
```{r}
get_timeslice_ci <- function(hr, util_df, z = 1.96) {
  filtered_df <- util_df %>%
    filter(hour == hr) 
  
  utilization <- filtered_df$utilization
  
  if(min(utilization, na.rm = TRUE) <= 0) warning("There are negative utilization rates in the dataframe.")
...
for(day in daytype_names) {
  ...
  this_CI_df <- sapply(hrs, get_timeslice_ci, util_df = this_df, simplify = 'array')
  
  # this_CI_df <- data.frame(this_CI_df)
  
  assign(this_CI_df, CI_df_name)
}

spr_wk_CI <- sapply(hrs, get_timeslice_ci, util_df = spr_wk_util, simplify = 'array')
